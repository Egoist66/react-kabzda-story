...
const Button: FC<ButtonProps> = ({isColoredBtn = false, isDisabled = false}) => {
  ...
  return (
    ...
    <button disabled={isDisabled} style={{backgroundColor: isColoredBtn || isColored ? 'red' : 'blue'}} onClick={makeButtonColor}>
      Click me
    </button>
  )
}
    ...
    <div>
      ...
      <div style={{display: 'flex', gap: 30, marginBottom: 20}}>
        <div>
          ...
          <input  onChange={() => setColor(true)} id={id1} type="radio" name="color" />
        </div>

        <div>
          <label htmlFor={id2}>Blue</label>
          <input  onChange={() => setColor(false)} id={id2} type="radio" name="color" />
        </div>
      </div>

      <Button isDisabled={false} isColoredBtn={isColored} />
